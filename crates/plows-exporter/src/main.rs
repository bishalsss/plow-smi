...
//!
//! Usage:
//!   plows-exporter --all                    # Enable all collectors
//!   plows-exporter --nvidia --system        # NVIDIA + system only
//!   plows-exporter --amd --intel --port 9100
...
use plows_exporter::collector::manager::CollectorManager;
use plows_exporter::config::Config;
use plows_exporter::exporter::prometheus::{health_handler, metrics_handler};
use plows_exporter::metrics::gpu_metrics::update_gpu_metrics;

#[cfg(feature = "system")]
use plows_exporter::collector::system::SystemCollector;
#[cfg(feature = "system")]
use plows_exporter::metrics::system_metrics::update_system_metrics;
...
async fn main() -> anyhow::Result<()> {
    ...
    // Register unified GPU collector (plows-gpu: NVIDIA / AMD / Intel)
    #[cfg(feature = "gpu")]
    {
